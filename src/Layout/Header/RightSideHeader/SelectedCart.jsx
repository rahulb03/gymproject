          ...
          <li className='product-box-contain' key={i}>
            <div className='drop-cart'>
              <Link href={`/${i18Lang}/product/${elem?.product?.slug}`} className='drop-image'>
                <Avatar data={ASSETS_URL + (elem?.variation?.variation_image ?? elem?.product?.productThumbnail)} placeHolder={placeHolderImage} name={elem?.product?.productName} height={72} width={87} />
              </Link>

              <div className='drop-contain'>
                <Link href={`/${i18Lang}/product/${elem?.product?.slug}`}>
                  <h5 className='text-capitalize'>{elem?.variation?.name ?? elem?.product?.productName}</h5>
                </Link>
                <h6>{convertCurrency(elem?.variation?.sale_price ?? elem?.product?.productPrice)}</h6>

...
import { productData, productData2 } from '@/Config/Constant';
...
const CollectionProducts = ({ filter, grid }) => {
  ...
  //   },
  // );
  const fetchStatus = 'done'
  const data = [...productData, ...productData2]
    ...
    <>
      {fetchStatus == 'fetching' ? (
        ...
        <Row xxl={grid !== 3 && grid !== 5 ? 4 : ''} xl={grid == 5 ? 5 : 3} lg={grid == 5 ? 4 : 2} md={3} xs={2} className={`g-sm-4 g-3 product-list-section ${grid == 'list' ? 'list-style' : ''}`}>
          {data?.map((product, i) => (
            <Col key={i}>
              <ProductBox1 imgUrl={product?.productThumbnail} productDetail={{ ...product }} classObj={{ productBoxClass: 'product-box-3' }} />
            </Col>
          ))}
        </Row>
      ) : (
